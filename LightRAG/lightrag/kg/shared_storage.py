...
###################################################################
# DEFAULT_WORKSPACE = os.getenv("LIGHTRAG_WORKSPACE", "default")
####################################################################
...
def get_graph_db_lock():
    """
    Fallback lock for graph DB operations.

    Our current usage is single-process / single-threaded, so a no-op lock is
    enough to satisfy LightRAG's `with/async with get_graph_db_lock():` calls.
    """
    return _DummyLock()
